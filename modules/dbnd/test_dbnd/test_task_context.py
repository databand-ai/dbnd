...
class TestDatabandContext(object):
    def test_user_code_run(self):
        with new_dbnd_context(
            conf={"core": {"user_init": "test_dbnd.test_task_context._user_code"}}
        ):
            pass
    ...
    def test_user_config_inject(self):
        with new_dbnd_context(
            conf={
                "core": {"user_init": "test_dbnd.test_task_context.inject_some_params"},
    ...
    def test_user_code_fail(self):
        with pytest.raises(Exception, match=r"USER_CODE_ERROR"):
            with new_dbnd_context(
                conf={
                    "core": {
                        "user_init": "test_dbnd.test_task_context._user_code_raises"
                    }
                }
            ):
                pass
