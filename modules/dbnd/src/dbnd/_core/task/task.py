...
class Task(_BaseTask, _TaskParamContainer):
    ...
    * :py:meth:`output` - the output :py:class:`Target` that this Task creates.
    ...
    @classmethod
    def dbnd_run_airflow_operator(cls, name=None, cmd_line=None, **kwargs):
        from airflow.operators.bash_operator import BashOperator

        task_id = name or "run_%s" % cls.task_definition.task_family
        cmd_line = cmd_line or ""
        templated_command = "dbnd run {full_task_family} {cmd_line}".format(
            full_task_family=cls.task_definition.full_task_family, cmd_line=cmd_line
        )
    ...
    def dbnd_schedule_airflowDAG(
        ...
        **kwargs
        ...
        name = name or "scheduled_%s" % cls.task_definition.task_family
        ...
        cls.dbnd_run_airflow_operator(name=name, cmd_line=cmd_line, dag=dag)
        return dag
