...
%(complete_func)setup() {
    ...
    if [ ${BASH_VERSION_ARR[0]} -gt 4 ] || ([ ${BASH_VERSION_ARR[0]} -eq 4 ] && [ ${BASH_VERSION_ARR[1]} -ge 4 ]); then
        COMPLETION_OPTIONS="-o nosort -o nospace"
    fi
...
%(complete_func)s() {
    ...
    if [ -n "$completions_with_descriptions" ]; then
        if [[ "$is_suffix" == "true" ]]; then
          _describe -V unsorted completions_with_descriptions -U -Q -S "$suffix"
        else
...
zstyle ':completion:*:*:%(script_names)s:*:*' list-grouped no
'''
...
def do_complete(cli, prog_name, include_descriptions):
    ...
    for item in get_choices(cli, prog_name, args, incomplete):
        if not include_descriptions and item[0] == ZSH_COMPLETION_SUFFIX:
            continue
