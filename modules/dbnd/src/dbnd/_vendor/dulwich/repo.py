...
from dbnd._vendor.dulwich.object_store import (
    DiskObjectStore,
    MemoryObjectStore,
...
from dbnd._vendor.dulwich.pack import pack_objects_to_data

from dbnd._vendor.dulwich.hooks import (
    PreCommitShellHook,
    PostCommitShellHook,
    CommitMsgShellHook,
)

from dbnd._vendor.dulwich.refs import (  # noqa: F401
    check_ref_format,
    RefsContainer,
    DictRefsContainer,
    InfoRefsContainer,
...
class BaseRepo(object):
    ...
    def fetch_objects(
        ...
        return self.object_store.iter_shas(
            self.object_store.find_missing_objects(
                haves, wants, progress, get_tagged, get_parents=get_parents
            )
        )
    ...
    def get_walker(self, include=None, *args, **kwargs):
        ...
        """
        from dbnd._vendor.dulwich.walk import Walker
...
class Repo(BaseRepo):
    ...
    def __init__(self, root):
        ...
        self.path = root
        object_store = DiskObjectStore(os.path.join(self.commondir(), OBJECTDIR))
    ...
    def _write_reflog(
        ...
        path = os.path.join(
            self.controldir(), "logs", ref.decode(sys.getfilesystemencoding())
        )
        try:
    ...
    def open_index(self):
        ...
        :return: The matching `Index`
        """
        from dbnd._vendor.dulwich.index import Index
    ...
    def stage(self, fs_paths):
        ...
        if not isinstance(fs_paths, list):
            fs_paths = [fs_paths]
        from dbnd._vendor.dulwich.index import (
