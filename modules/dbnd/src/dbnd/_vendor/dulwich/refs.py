...
from dbnd._vendor.dulwich.file import (
    GitFile,
    ensure_dir_exists,
)
...
class RefsContainer(object):
    ...
    def import_refs(
        self, base, other, committer=None, timestamp=None, timezone=None, message=None
    ):
...
class DiskRefsContainer(RefsContainer):
    ...
    def subkeys(self, base):
        ...
        for root, unused_dirs, files in os.walk(path):
            dir = root[len(path) :]
            if os.path.sep != "/":
                dir = dir.replace(os.path.sep.encode(sys.getfilesystemencoding()), b"/")
            dir = dir.strip(b"/")
    ...
    def refpath(self, name):
        ...
        """
        if os.path.sep != "/":
            name = name.replace(b"/", os.path.sep.encode(sys.getfilesystemencoding()))
