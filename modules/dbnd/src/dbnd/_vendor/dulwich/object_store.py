...
from dbnd._vendor.dulwich.errors import NotTreeError
...
from dbnd._vendor.dulwich.pack import (
    Pack,
    PackData,
    PackInflater,
    iter_sha1,
...
from dbnd._vendor.dulwich.refs import ANNOTATED_TAG_SUFFIX
...
class BaseObjectStore(object):
    ...
    def find_missing_objects(
        ...
        finder = MissingObjectFinder(
            self, haves, wants, progress, get_tagged, get_parents=get_parents
        )
    ...
    def generate_pack_data(self, have, want, progress=None, ofs_delta=True):
        ...
        return pack_objects_to_data(self.generate_pack_contents(have, want, progress))
    ...
    def _collect_ancestors(
        self, heads, common=set(), get_parents=lambda commit: commit.parents
    ):
    ...
    def _update_pack_cache(self):
        try:
            ...
            if e.errno == errno.ENOENT:
                ...
                return
            raise
        self._pack_cache_time = max(os.stat(self.pack_dir).st_mtime, time.time())
        pack_files = set()
...
class MissingObjectFinder(object):
    ...
    def __init__(
        ...
        all_ancestors = object_store._collect_ancestors(
            have_commits, get_parents=self._get_parents
        )[0]
