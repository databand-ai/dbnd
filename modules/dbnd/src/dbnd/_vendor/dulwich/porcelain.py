...
from dbnd._vendor.dulwich.archive import tar_stream
from dbnd._vendor.dulwich.client import get_transport_and_path
from dbnd._vendor.dulwich.config import StackedConfig
...
from dbnd._vendor.dulwich.errors import SendPackError, UpdateRefsError
from dbnd._vendor.dulwich.ignore import IgnoreFilterManager
from dbnd._vendor.dulwich.index import blob_from_path_and_stat, get_unstaged_changes
from dbnd._vendor.dulwich.object_store import tree_lookup_path
...
from dbnd._vendor.dulwich.objectspec import (
    parse_commit,
    parse_object,
    parse_ref,
    parse_reftuples,
...
from dbnd._vendor.dulwich.pack import write_pack_index, write_pack_objects
from dbnd._vendor.dulwich.patch import write_tree_diff
from dbnd._vendor.dulwich.protocol import ZERO_SHA, Protocol
from dbnd._vendor.dulwich.refs import ANNOTATED_TAG_SUFFIX, strip_peeled_refs
from dbnd._vendor.dulwich.repo import BaseRepo, Repo
from dbnd._vendor.dulwich.server import (
    FileSystemBackend,
    ReceivePackHandler,
    TCPGitServer,
    UploadPackHandler,
...
default_bytes_out_stream = getattr(sys.stdout, "buffer", sys.stdout) or NoneStream()
default_bytes_err_stream = getattr(sys.stderr, "buffer", sys.stderr) or NoneStream()
...
def add(repo=".", paths=None):
    ...
    with open_repo_closing(repo) as r:
        ...
        if not paths:
            paths = list(get_untracked_paths(os.getcwd(), r.path, r.open_index()))
        relpaths = []
...
def show_commit(repo, commit, decode, outstream=sys.stdout):
    ...
    diffstream.seek(0)
    outstream.write(
        diffstream.getvalue().decode(commit.encoding or DEFAULT_ENCODING, "replace")
    )
    ...
    **kwargs
    ...
    with open_repo_closing(repo) as r:
        ...
        try:
            ...
            errstream.write(
                b"Push to "
                + remote_location_bytes
                + b" failed -> "
                + e.message.encode(err_encoding)
                + b"\n"
            )
    ...
    for dirpath, dirnames, filenames in os.walk(frompath):
        ...
        if ignore_manager:
            for dirname in dirnames[:]:
                if ignore_manager.is_ignored(os.path.join(dirpath, dirname)):
...
def web_daemon(path=".", address=None, port=None):
    ...
    """
    from dbnd._vendor.dulwich.web import (
...
def fetch(
    ...
    **kwargs
    ...
    with open_repo_closing(repo) as r:
        ...
        r.refs.import_refs(b"refs/remotes/" + remote_name, branches, message=message)
        tags = {
            n[len(b"refs/tags/") :]: v
            for (n, v) in stripped_refs.items()
...
def check_mailmap(repo, contact):
    ...
    with open_repo_closing(repo) as r:
        ...
        from dbnd._vendor.dulwich.mailmap import Mailmap
