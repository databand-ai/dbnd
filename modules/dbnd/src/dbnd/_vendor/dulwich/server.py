...
from dbnd._vendor.dulwich import log_utils
from dbnd._vendor.dulwich.objects import (
    Commit,
    valid_hexsha,
)
from dbnd._vendor.dulwich.pack import write_pack_objects
from dbnd._vendor.dulwich.protocol import (  # noqa: F401
...
from dbnd._vendor.dulwich.refs import (
...
class PackHandler(Handler):
        ...
        for cap in caps:
            if cap not in allowable_caps:
                raise GitProtocolError(
                    "Client asked for capability %s that " "was not advertised." % cap
                )
        for cap in self.required_capabilities():
            if cap not in caps:
                raise GitProtocolError(
                    "Client does not support required " "capability %s." % cap
                )
    ...
    def has_capability(self, cap):
        if self._client_capabilities is None:
            raise GitProtocolError(
                "Server attempted to access capability %s " "before asking client" % cap
            )
...
class UploadPackHandler(PackHandler):
    ...
    def __init__(self, backend, args, proto, http_req=None, advertise_refs=False):
        super(UploadPackHandler, self).__init__(backend, proto, http_req=http_req)
...
class ReceivePackHandler(PackHandler):
    ...
    def __init__(self, backend, args, proto, http_req=None, advertise_refs=False):
        super(ReceivePackHandler, self).__init__(backend, proto, http_req=http_req)
        self.repo = backend.open_repository(args[0])
        self.advertise_refs = advertise_refs
