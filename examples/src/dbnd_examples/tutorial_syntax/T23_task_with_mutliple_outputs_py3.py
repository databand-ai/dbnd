...
def func_returns_two_dataframes_v1(p: int) -> (DataFrame, DataFrame):
    return (
...
@task
def func_returns_two_dataframes_no_hint(p: int) -> (DataFrame, DataFrame):
    return (
        pd.DataFrame(data=[[p, 1]], columns=["c1", "c2"]),
        pd.DataFrame(data=[[p, 1]], columns=["c1", "c2"]),
    )
...
@task
def func_returns_two_dataframes_named_tuple_v1(p: int) -> FeatureStore:
    return FeatureStore(
        pd.DataFrame(data=[[p, 1]], columns=["c1", "c2"]),
        pd.DataFrame(data=[[p, 1]], columns=["c1", "c2"]),
    )
