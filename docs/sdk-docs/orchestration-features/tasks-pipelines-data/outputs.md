...
* The `train_model` task returns an ElasticNet object which is serialized into a pickle and is given a default name of `result`. 
...
By default, `{root}` is set to `$DBND_HOME\data` and `{env}` is equal to `dev`. The exact path of task outputs is available in logs and the console. For example, this is a path generated by databand: `"/data/dev/2021-07-02/fetch_wine_quality/fetch_wine_quality_6236128077/result.csv"`
...
For example, running `train_model` task on the same data set from 4th of August 2018, with default `local` environment configuration would produce the following artifacts:

-   `$DBND_HOME/dbnd/data/dev/date=2018-08-04` directory, if such directory doesn't yet exist
-   `TrainModel_9574807509` directory under `$DBND_HOME/dbnd/data/dev/date=2018-08-04` directory (the numerical value after the task name is a task signature)
-   The task output (i.e., a trained model) would be stored within the `TrainModel_9574807509` directory
-   An additional meta-directory under `TrainModel_9574807509`. DBND will use this directory to store run info metadata including metrics, git commit and other artifacts.
